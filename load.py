...
colorsUsed = pd.read_csv('./TJOP-ColorsUsed.csv')
subjectMatter = pd.read_csv('./TJOP-Subject Matter.csv')

print(subjectMatter)
print(type(subjectMatter))
print(subjectMatter.__dict__)

episodeDates = [re.findall(r'[\w\s]\(([\d\w,\s]+)\)[$ ]?', line)[0] for line in open('TJOP-Eps Dates')]
print(episodeDates)
